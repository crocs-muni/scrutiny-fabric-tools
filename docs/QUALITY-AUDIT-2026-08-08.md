...
| Agentic SDLC governance (Step 5 post) | ✅ | Root `AGENTS.md` (command-first contract: board protocol, autonomy ladder, refute-or-promote posture, mutation-testing caveats, handoff convention). `.github/PULL_REQUEST_TEMPLATE.md` (evidence block). `packages/core/tools/mutant-check.mjs` + `pnpm mutant-check` (survivor hand-check harness — offset-spliced full-suite verification before any "equivalent mutant" justification). `docs/spec-changes/` index (one file per F-finding, status-tracked: proposed/applied/archived). Board labels (`in-progress`/`blocked`/`icelog`). Inspired by [RedHatResearch/aibom-security](https://github.com/RedHatResearch/aibom-security)'s AGENTS.md model; see §3 Step-5 post for the research digest |
...
- [x] Step 6 — browser memory investigation, **closed 2026-08-10** (closes `AUDIT-2026-07-31.md` hazard #4). Full write-up: [`docs/BROWSER-MEMORY-2026-08-10.md`](BROWSER-MEMORY-2026-08-10.md). Real sec-certs corpus (6,737 certs, 202 MB) mapped to **172,430 events / 120.5 MB wire (0.683 KB/event)**; measured on Node 25.6.1 (V8 14.1) and headless Chrome 151 (V8 15.1, ≥120) through `createStore`'s public API, 2 loads per size. Chrome retains the raw corpus at **0.97–0.99 KB/event vs Node's 1.30–1.35** (25% lighter, pointer-compression direction §4 predicted, now measured); store-retained heap stays within ~0.5 MB of corpus-retained in Chrome (sandbox-attribution gap noted, renderer WS recorded). Verdict: **in-memory posture is safe at corpus scale** — even the full 172,430-event corpus fits a mobile-class tab ceiling with headroom; the binding constraint is **ingest time, not heap** (112,565 events took 303 s in Node / 204 s in Chrome — quadratic in patches×observed; profiled to `eTags` re-parsing + `admit.resync()` full-set re-walks; not the depth-64 pathology). `appliedCompat` (criterion 1) is a **negative finding** — no such option exists in repo, spec, flags, or web search; measured without it. New finding filed as issue [#34](https://github.com/crocs-muni/scrutiny-fabric-tools/issues/34) (separate, not blocking; closes Step 6's performance loop)
